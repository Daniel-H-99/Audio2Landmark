...
class Audio2Exp(nn.Module):
    def __init__(self, params, is_train=True):
        ...
        self.norm = nn.LayerNorm((4, self.input_dim))

    def init_hidden(self, params):
        # Create the trainable initial state
        h_init = init.constant_(torch.empty(params['lstm']['layer_num'], self.batch_size, params['lstm']['node_dim'], dtype=torch.float32), 0.0)
        c_init = init.constant_(torch.empty(params['lstm']['layer_num'], self.batch_size, params['lstm']['node_dim'], dtype=torch.float32), 0.0)
        self.hidden = (h_init.to(params['device']), c_init.to(params['device']))
...
#         audio = self.norm(audio)
        output, _ = self.lstm(audio, (self.hidden[0].clone(), self.hidden[1].clone()))
#         print(output)
        output == self.dropout(output)
